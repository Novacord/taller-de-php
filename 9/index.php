...
<input type="text" placeholder="Ingrese en numero de alienígenas" name="numero" autocomplete="off"/>
...
if (isset($_POST['numero'])){
  ...
  for ($i = 1; $i <= intval($_POST['numero']); $i++) {
    $inputs .= '
    <input type="text" placeholder="Nombre del alienígena" name="alienigena[]" autocomplete="off"/>
    ';
  }
...
if (isset($_POST['alienigena'])){
  $filtrar = array_unique($_POST['alienigena']);
  $form = '';
  foreach ($filtrar as $key => $NombreP) {
    $form .= '<div class="login-form">
    <P class="mesage"> alienigena '.$NombreP.'</P>
    </div>';
  };
};
